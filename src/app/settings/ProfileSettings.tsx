...
export const ProfileSettings = () => {
  ...
  const handleGetPerfil = async () => {
    try {
      ...
      setCuelloManga(
        perfilActual.cuelloManga ? perfilActual.cuelloManga.toString() : ""
      );
      ...
      setEntrepierna(
        perfilActual.entrepierna ? perfilActual.entrepierna.toString() : ""
      );
      setError(null);
    } catch (error) {
      setError(
        `Error al cargar el perfil. Por favor, inténtalo de nuevo. ${error}`
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (cuelloManga !== "" && !isNaN(Number(cuelloManga)))
        data.cuelloManga = Number(cuelloManga);
      if (pecho !== "" && !isNaN(Number(pecho))) data.pecho = Number(pecho);
      if (cintura !== "" && !isNaN(Number(cintura)))
        data.cintura = Number(cintura);
      if (cadera !== "" && !isNaN(Number(cadera))) data.cadera = Number(cadera);
      if (entrepierna !== "" && !isNaN(Number(entrepierna)))
        data.entrepierna = Number(entrepierna);
