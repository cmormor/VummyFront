...
import { deleteClothe, getClotheById, getClothes, postClothe } from "../../api/clotheApi";

interface ClotheFormData {
    id?: number;
    nombre: string;
    precio: number;
    descripcion: string;
...
export const ClotheSettings = () => {
    ...
    const [selectedClothe, setSelectedClothe] = useState<ClotheFormData | null>(null);
    const [formData, setFormData] = useState<ClotheFormData>({
        nombre: '',
        precio: 0,
        descripcion: '',
        stock: 0,
    ...
    useEffect(() => {
        if (!searchTerm.trim()) {
            ...
            const filtered = clotheList.filter(clothe =>
                clothe.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
                clothe.precio.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                clothe.stock.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                clothe.tallaNombre.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                clothe.tiendaId.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
                clothe.tiendaNombre.toLowerCase().includes(searchTerm.toLowerCase())
            );
    ...
    const handleInputChange = (field: keyof ClotheFormData, value: any) => {
        setFormData(prev => ({
            ...prev,
            [field]: value
        }));
    ...
    const handleCreate = async (clothe: Clothe) => {
        const response = await postClothe(clothe);
        return response;
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            const updatedList = clotheList.filter(clothe => clothe.id !== id);
            setClotheList(updatedList);
            setFilteredClothe(updatedList);
                            ...
                            <TableBody>
                                {filteredClothe.map((clothe) => (
                                    <TableRow key={clothe.id} hover>
                                        ...
                                        <TableCell>{clothe.stock}</TableCell>
